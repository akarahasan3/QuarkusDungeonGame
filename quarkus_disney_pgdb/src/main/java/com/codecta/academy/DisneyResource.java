...
public class DisneyResource {
    ...
    @Path("/themeparks/{id}")
    @Produces(MediaType.APPLICATION_JSON)
    public Response updateThemeParkById(@PathParam("id") Integer id, ParkDto park)
    {
        ParkDto parkDto = disneyService.updateThemePark(id, park);
        if(parkDto == null) {
            return Response.status(Response.Status.NOT_FOUND).build();
        }
        return Response.ok(parkDto).build();
    }
    ...
    @Produces(MediaType.APPLICATION_JSON)
    public Response getCharactersByCharacter(@QueryParam("name") String name, @QueryParam("greeting") String greeting){
        List<CharacterDto> characterDtoList = disneyService.findCharacterByCharacter(name, greeting);
