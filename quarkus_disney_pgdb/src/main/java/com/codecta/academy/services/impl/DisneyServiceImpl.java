...
public class DisneyServiceImpl implements DisneyService {
    ...
    @Override
    public List<CharacterDto> findCharacterByCharacter(String name, String greeting) {
        List<DisneyCharacter> disneyCharactersList = disneyCharacterRepository.findByCharacter(name, greeting);
        if(disneyCharactersList == null || disneyCharactersList.isEmpty()) {
            return null;
        }
        List<CharacterDto> characterDtoList = new ArrayList<>();
        ModelMapper modelMapper = new ModelMapper();
